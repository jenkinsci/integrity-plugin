      ...
      throws InterruptedException, IOException
  {
    ...
    if (!(rootProject.getScm() instanceof IntegritySCM))
    {
      listener.getLogger().println(
          "Integrity Item update is being executed for an invalid context!  Current SCM is "
              + rootProject.getScm() + "!");
      return true;
    }
    ...
    try
    {
      ...
      if (intBuildItemID <= 0)
      {
        if (queryDefinition.length() > 0)
        {
          ...
          if (null != issuesResponse)
          {
            ...
            {
              listener.getLogger().println(
                  "Cannot find an Integrity Build Item!  Response from executing custom query is null!");
              return false;
            }
          } else
          {
        ...
        {
          listener.getLogger().println(
              "WARNING: No configuration information provided to locate an Integrity Build Item!");
        }
      }
      ...
      if (null != testResult && testResult.getTotalCount() > 0)
      {
        ...
        {
          listener.getLogger().println("Obtained Integrity Test Session Item '" + testSessionID
              + "' from build environment!");
          success = collectTestResults(build, listener, testSessionID);
          listener.getLogger().println("Updated Integrity Test Session Item '" + testSessionID
      ...
      {
        listener.getLogger()
            .println("Obtained Integrity Build Item '" + buildItemID + "' from build environment!");
