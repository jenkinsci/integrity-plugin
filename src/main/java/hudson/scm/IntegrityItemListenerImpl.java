...
public class IntegrityItemListenerImpl extends ItemListener
{
  ...
  public void onCopied(Item src, Item tgt)
  {
    ...
    if (src instanceof AbstractProject && tgt instanceof AbstractProject)
    {
      ...
      if (srcProject.getScm() instanceof IntegritySCM
          && tgtProject.getScm() instanceof IntegritySCM)
      {
        ...
        {
          LOGGER
              .fine("Current configuration name is not unique - " + tgtSCM.getConfigurationName());
          tgtSCM.setConfigurationName(
              ((IntegritySCM.DescriptorImpl) tgtSCM.getDescriptor()).getConfigurationName());
          LOGGER.info(
              "Resetting Integrity 'Configuration Name' to - " + tgtSCM.getConfigurationName());
        }
      }
    }
  }
  ...
  public void onDeleted(Item item)
  {
    ...
    try
    {
      List<String> jobsList =
          DerbyUtils.getDistinctJobNames(DescriptorImpl.INTEGRITY_DESCRIPTOR.getDataSource());
      if (jobsList.contains(item.getName()))
      {
        DerbyUtils.deleteProjectCache(DescriptorImpl.INTEGRITY_DESCRIPTOR.getDataSource(),
            item.getName());
      }
