...
public class IntegrityChangeLogParser extends ChangeLogParser
{
  ...
  /**
   * Provides a mechanism to obtain the host/port information for constructing links to annotations
   * and differences views
   */
   ...
   */
  @Override
  public IntegrityChangeLogSet parse(@SuppressWarnings("rawtypes") Run run,
      RepositoryBrowser<?> browser, File changeLogFile) throws IOException, SAXException
  {
    List<IntegrityChangeLog> changeSetList = new ArrayList<IntegrityChangeLog>();
    Digester digester = new Digester();
    digester.push(changeSetList);

    // When digester reads a {{<items>}} child node of {{<changelog>}} it will create a
    // {{IntegrityChangeLog}} object
