...
public class ISessionPool
{
  ...
  {
    LOGGER.log(Level.FINE, "Starting PTC RV&S Session Pool");
    startPool();
  }
      ...
      extends BaseKeyedPooledObjectFactory<IntegrityConfigurable, ISession>
  {
    ...
    {
      LOGGER.log(Level.FINE, "Creating a new PTC RV&S Session for the Session Pool :"
          + settings.getConfigId() + " :: " + settings.toString());
      ISession api = APISession.create(settings);
      if (null == api)
      {
        LOGGER.log(Level.SEVERE, "An PTC RV&S API Session could not be established :"
            + settings.getConfigId() + " :: " + settings.toString());
        throw new AbortException("An PTC RV&S API Session could not be established :"
            + settings.getConfigId() + " :: " + settings.toString());
      }
      return api;
    }
    ...
    {
      LOGGER.log(Level.FINEST, "Terminating PTC RV&S Session Pool object : " + key.getConfigId()
          + " :: " + key.toString());
    ...
    {
      LOGGER.log(Level.FINEST, "Validating PTC RV&S Session Pool object : " + key.getConfigId()
          + " :: " + key.toString());
      ...
      {
        LOGGER.log(Level.FINEST, "Failed to ping PTC RV&S Session Pool object : "
